// functions/robotics-capex.js
//
// The Robotics view capex map — same KV-backed pattern as /capex and
// /musk-capex, separate key. GET: public read (null → frontend default).
...
export async function onRequest(context) {
  ...
  if (request.method === "GET") {
    try {
      const data = env.SHARED_DATA ? await env.SHARED_DATA.get("roboticsCapexData", "json") : null;
  ...
  if (request.method === "POST") {
    try {
      ...
      if (env.SHARED_DATA && body.capexData) {
        ...
        await env.SHARED_DATA.put("roboticsCapexData", JSON.stringify(payload));
